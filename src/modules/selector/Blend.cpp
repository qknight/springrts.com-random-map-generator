...
NoiseNetwork* Blend::network() {
    ...
    if (ready()) {
        ...
        noise::module::Blend* m_Blend = new noise::module::Blend();
        noiseNetwork->addModule(m_Blend);

        //   3. assign all properties
        //   m_Blend->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Blend->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Blend->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
