...
NoiseNetwork* Spheres::network() {
    ...
    if (ready()) {
        ...
        noise::module::Spheres* m_Spheres = new noise::module::Spheres();
        noiseNetwork->addModule(m_Spheres);

        //   3. assign all properties
        //   m_Spheres->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Spheres->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Spheres->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
