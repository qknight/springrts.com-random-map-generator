...
NoiseNetwork* Checkerboard::network() {
    ...
    if (ready()) {
        ...
        noise::module::Checkerboard* m_Checkerboard = new noise::module::Checkerboard();
        noiseNetwork->addModule(m_Checkerboard);

        //   3. assign all properties
        //   m_Checkerboard->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Checkerboard->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Checkerboard->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
