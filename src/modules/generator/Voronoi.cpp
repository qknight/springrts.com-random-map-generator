...
NoiseNetwork* Voronoi::network() {
    ...
    if (ready()) {
        ...
        noise::module::Voronoi* m_Voronoi = new noise::module::Voronoi();
        noiseNetwork->addModule(m_Voronoi);

        //   3. assign all properties
        //   m_Voronoi->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Voronoi->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Voronoi->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
