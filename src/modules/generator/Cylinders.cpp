...
NoiseNetwork* Cylinders::network() {
    ...
    if (ready()) {
        ...
        noise::module::Cylinders* m_Cylinders = new noise::module::Cylinders();
        noiseNetwork->addModule(m_Cylinders);

        //   3. assign all properties
        //   m_Cylinders->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Cylinders->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Cylinders->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
