...
NoiseNetwork* Multiply::network() {
    ...
    if (ready()) {
        ...
        noise::module::Multiply* m_Multiply = new noise::module::Multiply();
        noiseNetwork->addModule(m_Multiply);

        //   3. assign all properties
        //   m_Multiply->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Multiply->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Multiply->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
