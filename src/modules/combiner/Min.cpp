...
NoiseNetwork* Min::network() {
    ...
    if (ready()) {
        ...
        noise::module::Min* m_Min = new noise::module::Min();
        noiseNetwork->addModule(m_Min);

        //   3. assign all properties
        //   m_Min->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Min->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Min->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
