...
NoiseNetwork* Power::network() {
    ...
    if (ready()) {
        ...
        noise::module::Power* m_Power = new noise::module::Power();
        noiseNetwork->addModule(m_Power);

        //   3. assign all properties
        //   m_Power->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Power->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Power->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
