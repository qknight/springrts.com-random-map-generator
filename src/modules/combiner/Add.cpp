...
NoiseNetwork* Add::network() {
    ...
    if (ready()) {
        ...
        noise::module::Add* m_Add = new noise::module::Add();
        noiseNetwork->addModule(m_Add);

        //   3. assign all properties
        //   m_Add->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Add->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Add->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
