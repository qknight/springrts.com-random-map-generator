...
NoiseNetwork* RotatePoint::network() {
    ...
    if (ready()) {
        ...
        noise::module::RotatePoint* m_RotatePoint = new noise::module::RotatePoint();
        noiseNetwork->addModule(m_RotatePoint);

        //   3. assign all properties
        //   m_RotatePoint->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_RotatePoint->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_RotatePoint->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
