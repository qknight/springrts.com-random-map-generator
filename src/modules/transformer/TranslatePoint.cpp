...
NoiseNetwork* TranslatePoint::network() {
    ...
    if (ready()) {
        ...
        noise::module::TranslatePoint* m_TranslatePoint = new noise::module::TranslatePoint();
        noiseNetwork->addModule(m_TranslatePoint);

        //   3. assign all properties
        //   m_TranslatePoint->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_TranslatePoint->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_TranslatePoint->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
