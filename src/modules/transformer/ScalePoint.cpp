...
NoiseNetwork* ScalePoint::network() {
    ...
    if (ready()) {
        ...
        noise::module::ScalePoint* m_ScalePoint = new noise::module::ScalePoint();
        noiseNetwork->addModule(m_ScalePoint);

        //   3. assign all properties
        //   m_ScalePoint->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_ScalePoint->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_ScalePoint->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
