...
NoiseNetwork* ScaleBias::network() {
    ...
    if (ready()) {
        ...
        noise::module::ScaleBias* m_ScaleBias = new noise::module::ScaleBias();
        noiseNetwork->addModule(m_ScaleBias);

        //   3. assign all properties
        //   m_ScaleBias->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_ScaleBias->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_ScaleBias->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
