...
NoiseNetwork* Exponent::network() {
    ...
    if (ready()) {
        ...
        noise::module::Exponent* m_Exponent = new noise::module::Exponent();
        noiseNetwork->addModule(m_Exponent);

        //   3. assign all properties
        //   m_Exponent->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Exponent->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Exponent->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
