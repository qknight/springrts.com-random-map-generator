...
NoiseNetwork* Abs::network() {
    ...
    if (ready()) {
        ...
        noise::module::Abs* m_Abs = new noise::module::Abs();
        noiseNetwork->addModule(m_Abs);

        //   3. assign all properties
        //   m_Abs->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Abs->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Abs->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
