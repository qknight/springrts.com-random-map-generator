...
NoiseNetwork* Clamp::network() {
    ...
    if (ready()) {
        ...
        noise::module::Clamp* m_Clamp = new noise::module::Clamp();
        noiseNetwork->addModule(m_Clamp);

        //   3. assign all properties
        //   m_Clamp->SetOctaveCount (property("OctaveCount"));
            ...
            if (chi->getObjectType() == DataItemType::PORT) {
                ...
                if (p->PortDirection() == PortDirection::IN || p->PortDirection() == PortDirection::MOD) {
                    ...
                    if (p->PortDirection() == PortDirection::IN) {
                      ...
                      m_Clamp->SetSourceModule(p->PortNumber(), n->topLevelModule());
                      noiseNetwork += n;
                    }
                    */
                    /*
                    if (p->PortDirection() == PortDirection::MOD) {
                      NoiseNetwork* n = module->network();
                      m_Clamp->SetControlModule(n->topLevelModule());
                      noiseNetwork += n;                      
                    }
                    */
                }
