...
visualModuleItem::~visualModuleItem() {
  if (children().size() > 0) {
    // when removing a graphical item, this must always be done from the lower layer (moduleFactory) and
    // the moduleFactory should remove connections before removing objects (depending on connections) first.
